...
class TravelForm(FlaskForm):
    destination = SelectField(label='Destino:', choices=
                               [], default='Selecciona un destino', validators=[DataRequired()])
...
def populate(travel_form):
    ...
    # destination travel options
    travel_form.destination.choices = [(str(i), str(destination[i])) for i in destination]
...
def consult():
    ...
    people = request.form.get('days')
    days = request.form.get('way_transport')
    way = request.form.get('type_trip')

    aux = []
    routes = []
    # aux = 
    # app.logger.info(lista_ciudades[int(destino)]["CPRO"] + lista_ciudades[int(destino)]["CMUN"])
    ...
    return redirect(url_for('hello_admin')) #, destino=destino, people=people, days=days, way=way))
